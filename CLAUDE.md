...
- **The wall is the castellum** (`docs/DESIGN.md` §4.4). Its tier decides what the circuit is made of: a bank, a palisade, coursed stone, then a crenellated circuit that flies a standard over the gate. Never add a separate wall building — the v0 building list in §4.4 is ✅ confirmed, so adding to it is Mathias's call.
