...
- **The wall is the castellum.** `data/buildings.json` calls that "garrison and walls", so its tier decides what the circuit is made of: a bank, a palisade, stone, then a crenellated circuit with towers. Never a separate wall building — that would be a new building, and §14 rules apply.
