...
If no round has run in N real hours, an **idle round** runs automatically: opponents act, the player does not. This closes the exploit where a player never opens the council and therefore never ages, is never raided and never faces a challenge. ✅ N = 24 hours (`data/config.json` `calendarFloorHours`). ✅ After a long absence at most 7 idle rounds run (`idleRoundsMaxCatchUp`), so a month away is not thirty raids. The cap applies on load; whatever of the absence lies beyond it is forgiven, not queued for later ticks.

✅ *2026-09-24:* the return is told on **one card**, "While you were away": a tally, one ledger from before the first idle round to after the last, and the rounds' lines as one list. It is never a stack of round cards. A choice an idle round raised is asked on that card, not on a card of its own in front of it. Each round, with its report cards, stays under Reports; what the village clock did in the gap is on the Village tab's return strip.
...
v0.2.16 — 2026-09-24 — §3.3: coming back after an absence raises one summary card rather than a Round card and a digest of seven expanded rounds; a choice raised while away is asked on it. The 7-round cap on load is covered by tests for a week, ten days and a month away.
