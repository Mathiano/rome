...
This read, from 2026-09-15: *"If no round has run in N real hours, an idle round runs automatically: opponents act, the player does not."* It ran up to 7 such rounds on load (`calendarFloorHours` 24, `idleRoundsMaxCatchUp` 7). It existed to close the exploit where a player never convenes and so never ages, is never raided and never faces a challenge. That exploit is open again by this ruling; the colony still produces while the council waits. Both config values, the round-runner and every surface that spoke of the idle round are removed. Old saves drop its bookkeeping on load, and rounds that already ran without the player keep their mark in Reports.

🟡 CLAUDE.md and §2 do not yet state this as a pillar; Mathias called it "the absence pillar" on 2026-09-25. Adding it to §2 is his call.
