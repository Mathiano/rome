...
import { leaderOf, playerFamily } from './characters';
...
export function bribe(state: GameState, familyId: string): void {
  ...
  const { cost, attitude, gravitasLoss } = config.intrigue.bribe;
  ...
  if (leader) leader.gravitas = Math.max(0, leader.gravitas - gravitasLoss);
